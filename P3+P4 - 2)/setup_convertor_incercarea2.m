%%
% Nume si prenume: TODO
%

clearvars
clc

%% Magic numbers (replace with received numbers)
m = 2;
n = 2;

%% Process data and experiment setup (fixed, do not modify)
u_star = 0.15+n*0.045; % trapeze + PRBS amplitudes
delta = 0.02;
delta_spab = 0.015;

E = 12;  % converter source voltage

umin = 0; umax = 0.98; % input saturation
assert(u_star < umax-0.1)
ymin = 0; ymax = 1/(1-u_star)*E*2; % output saturation

% circuit components + parasitic terms
R = 15;
rL = 10e-3;
rC = 0.2;
rDS1 = 0.01;
rDS2 = 0.01;
Cv = 600e-6/3*m;
Lv = 40e-3*3/m;

% (iL0,uC0)
rng(m+10*n)
x0_slx = [(-1)^(n+1)*E/R,E/3/(1-u_star)];

Ts = 1e-5*(1+2*(u_star-0.15)/u_star); % fundamental step size
Ts = round(Ts*1e6)/1e6;

% input white noise power and sampling time
whtn_pow_in = 1e-11*(Ts*1e4)/2; 
whtn_Ts_in = Ts*2;
whtn_seed_in = 23341+m+2*n;
q_in = (umax-umin)/pow2(11); % input quantizer (DAC)

% output white noise power and sampling time
whtn_pow_out = 1e-7*E*(Ts*1e4/50)*(1+(50*u_star)*(u_star-0.15))/3; 
whtn_Ts_out = Ts*2;
whtn_seed_out = 23342-m-2*n;
q_out = (ymax-ymin)/pow2(11); % output quantizer (ADC)

meas_rep = 13+ceil(n/2); % data acquisition hardware sampling limitation

%% Input setup (can be changed/replaced/deleted)
% Cu datele noastre
t1=0.05; 
N=5;
tr=0.08; % am imultit cu 4 ca sa vedem si ceva peturbatii
p= round(tr/N/Ts)
DeltaT = p*(2^N-1)*Ts; % am inmultit cu 2 ca sa modificam spab ul
[input_LUT_dSPACE,Tfin] = generate_input_signal(Ts,t1,DeltaT,N,p,u_star,delta,delta_spab);

%% Data acquisition (use t, u, y to perform system identification)
out = sim("convertor_R2022b_cu_from_workspace.slx");

t = out.tout;
u = out.u;
y = out.y;
    
subplot(211)
plot(t,u)
subplot(212)
plot(t,y)

%% System identification
%i1=18299;
%i2=47419;
%i3=65189;
%i4=94909;
%i1=16717;
%i2=44054;
%i3=58587;
%i4=85317;

i1=17682;
i2=43176;
i3=59221;
i4=84504;
N_dec=12;

t_id= t(i1:N_dec:i2);
u_id= u(i1:N_dec:i2);
y_id= y(i1:N_dec:i2);


u_id=u_id- mean(u_id); 

y_id=y_id- mean(y_id); 

t_vd= t(i3:N_dec:i4);
u_vd= u(i3:N_dec:i4);
y_vd= y(i3:N_dec:i4);

u_vd=u_vd- mean(u_vd);

y_vd=y_vd- mean(y_vd); 
%%
figure('Name', 'Date Identificare vs Validare', 'NumberTitle', 'off');


subplot(2,2,1)
plot(t_id, u_id); 
title('Intrare Identificare (u_{id})');
ylabel('Amplitudine');



subplot(2,2,3)
plot(t_id, y_id);
title('Iesire Identificare (y_{id})');
xlabel('Timp [s]'); 
ylabel('Amplitudine');





subplot(2,2,2)
plot(t_vd, u_vd);
title('Intrare Validare (u_{vd})');



subplot(2,2,4)
plot(t_vd, y_vd);
title('Iesire Validare (y_{vd})');
xlabel('Timp [s]');


dat_id = iddata(y_id,u_id,t_id(2)-t_id(1));
dat_vd = iddata(y_vd,u_vd,t_vd(2)-t_vd(1));
%%
model_arx=arx(dat_id,[16,15,5]); % avem 3 parametri de structura
figure,resid(model_arx,dat_vd)
figure,compare(model_arx,dat_vd)
%%
model_armax=armax(dat_id,[2,2,5,6])                  % (dat_id,[nA nB nC nd) avem 4 parametrri de structura nA nB nD - care i i am gasit mai sus + nC care se ia prin incercari (nA,nB,nC,nD)
model_armax_rafinat = pem(dat_id, model_armax);
figure,resid(model_armax_rafinat,dat_vd)
figure,compare(model_armax_rafinat,dat_vd)
%%
%Testam modelul IV4 ( instrument variables)
model_iv4=iv4(dat_id,[2 2 1]);
 figure,resid(model_iv4,dat_vd); 
 figure,compare(model_iv4,dat_vd);
%%
%Testamo modelul OE
model_oe=oe(dat_id,[2,2,6])
 figure,resid(model_oe,dat_vd)
 figure,compare(model_oe,dat_vd)
%%
% Testam modelul Box Jenkins
%model_bj=bj(dat_id,[4 4  5 4 3]);
model_bj=bj(dat_id,[4 4  5 4 3]);
modelbj=pem(dat_id,model_bj)
 figure,resid(modelbj,dat_vd);
 figure,compare(modelbj,dat_vd);
%%
model_n4sid=n4sid(dat_id,1:15); %matricea huncle
 figure ,resid(model_n4sid,dat_vd);
 figure, compare (model_n4sid,dat_vd);
%%
% daca nu merge cu niciun model, incercam sa folosim tehnici de RAFINARE 
%modificam DC ul de exemplu sau facem decimarea datelor cu N(numarul cu
%sagetile din grafic cate puncte is)

%%
% incercam cu un model in spatiul starilor
model_n4sid=n4sid(dat_id,1:15);
% figure,resid(model_n4sid,dat_vd)  
% figure,compare(model_n4sid,dat_vd)
 %%
model_ssest=ssest(dat_id,1:15);
figure,resid(model_ssest,dat_vd)
figure,compare(model_ssest,dat_vd)

%%
figure
step(model_armax_rafinat)
title('Raspunsul indicial (la treapta)')
grid on
z = zero(model_armax_rafinat)
p = pole(model_armax_rafinat)

%%
figure
step(modelbj)
title('Raspunsul indicial (la treapta)')
grid on
z = zero(modelbj)
p = pole(modelbj)
%%
figure
step(model_n4sid)
title('Raspunsul indicial (la treapta)')
grid on
z = zero(model_n4sid)
p = pole(model_n4sid)

%%
figure
step(model_ssest)
title('Raspunsul indicial (la treapta)')
grid on
z = zero(model_ssest)
p = pole(model_ssest)